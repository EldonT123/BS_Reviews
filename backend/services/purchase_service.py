...
def ensure_purchase_csv_exists():
    ...
    if not os.path.exists(PURCHASE_CSV_PATH):
        with open(PURCHASE_CSV_PATH, "w", newline="", encoding="utf-8") as csvfile:
...
def generate_transaction_id() -> str:
    """Generate a mock transaction ID (in production, this would come from payment processor)"""
...
def save_purchase(purchase: Purchase) -> bool:
    ...
    try:
        with open(PURCHASE_CSV_PATH, "a", newline="", encoding="utf-8") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
...
def add_tokens_to_user(user_email: str, tokens: int) -> bool:
    ...
    try:
        ...
        username, password_hash, tier, current_tokens = user
        new_token_balance = (int(current_tokens) if current_tokens else 0) + tokens
        user = (user_email, username, password_hash, tier, str(new_token_balance))
        # Update user's token balance
        # This assumes the user_service has a method to update tokens
...
def process_purchase_with_cad(
    ...
    Returns:
        tuple[bool, str, Optional[Purchase]]: (success, message, purchase_object)
    """
    ...
    try:
        ...
        return False, "Purchase completed but failed to apply benefits", purchase
...
def process_purchase_with_tokens(
    ...
    user_tokens = getattr(user, 'tokens', 0) or 0
    if user_tokens < purchase_item.price_tokens:
        return False, f"Insufficient tokens. You have {user_tokens} but need {purchase_item.price_tokens}", None
    ...
    try:
        ...
        deduct_success = user_service.deduct_tokens_from_user(user_email, purchase_item.price_tokens)
...
def get_user_purchase_history(user_email: str) -> list[Purchase]:
    ...
    try:
        with open(PURCHASE_CSV_PATH, "r", newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                if row["user_email"].lower() == user_email.lower():
                    purchase = Purchase(
                        ...
                        amount_cad=float(row["amount_cad"]) if row["amount_cad"] else None,
                        amount_tokens=int(row["amount_tokens"]) if row["amount_tokens"] else None,
                        payment_method_last4=row["payment_method_last4"] if row["payment_method_last4"] else None,
                        tokens_received=int(row["tokens_received"]) if row["tokens_received"] else None,
                        rank_upgrade=row["rank_upgrade"] if row["rank_upgrade"] else None,
                        purchase_date=datetime.fromisoformat(row["purchase_date"]),
                        status=row["status"],
                        transaction_id=row["transaction_id"] if row["transaction_id"] else None
                    )
