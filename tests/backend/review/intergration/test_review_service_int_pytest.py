...
def test_add_multiple_reviews_and_average(isolated_movie_env):
    ...
    assert avg_rating == 4.0, f"Average of 5, 3, and 4 should be 4.0, got {
        avg_rating}"
...
def test_add_review_to_real_data(temp_real_data_copy):
    ...
    # flaking error cannot be fixed or will break code
    assert new_count == initial_count + 1, f"Review count should increase by 1. Was {
        initial_count}, now {new_count}"
